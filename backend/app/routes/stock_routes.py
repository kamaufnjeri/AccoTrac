...
@stock_bp.route('/<str:user_id>/<string:company_id>/createstock', methods=['POST'])
def create_stock(company_id, user_id):
    data = request.get_json()

    if not data:
        return "", 204
    else:
        message, code, response_item = stock_controllers.create_stock(user_id, company_id, data)
...
stock_bp.route("<str:company_id/<str:stock_id>/updatestock", methods=["PUT"])
...
@stock_bp.route("/<str:company_id>/<str:stock_id>/getstock", methods=["GET"])
def get_stock(company_id, stock_id):
    message, code, resp_item = stock_controllers.get_stock(company_id, stock_id)

    return jsonify({"message": message, "response": resp_item}), code

@stock_bp.route("/<str:company_id/<str:stock_id>/deleteaccout", methods=["delete"])
